...
const SocialCard = ({
  ...
  const containerProps = {
    ...
    className: `relative group p-5 rounded-2xl backdrop-blur-md bg-black/40 border border-white/10 overflow-hidden flex flex-col h-full hover:border-white/20 transition-all duration-300`,
    ...(secondaryLink
      ? {}
  ...
  return (
    <CardContainer {...containerProps}>
      ...
      <div className="relative z-10 flex flex-col items-center text-center h-full">
        ...
        <h3 className="text-lg font-bold text-white mb-2 [font-family:'ProximaNova',sans-serif]">
          {title}
        </h3>
        <p className="text-gray-300 text-sm mb-4 leading-relaxed [font-family:'ProximaNova',sans-serif] flex-grow">
          {description}
        </p>
        ...
        {secondaryLink ? (
          <div className="flex gap-3 w-full mt-auto">
            <a
              ...
              className="inline-flex justify-center items-center text-sm font-semibold tracking-wider uppercase text-white/80 hover:text-white transition-colors bg-white/5 py-3 rounded-xl hover:bg-white/10 flex-1"
            >
    ...
    <div className="bg-[linear-gradient(180deg,#4F4B60_0%,#6D6886_20%,#575078_34%,#36397E_98%)] min-h-screen text-white overflow-hidden pb-20 relative">
      ...
      <section className="container mx-auto px-4 relative z-10 max-w-[1450px]">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-5"
        >
          {/* Instagram */}
          <SocialCard
            icon={FaInstagram}
